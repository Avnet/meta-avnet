...
SZG_STD_LB_BASE_O=$BASE_SZG_STD_LB
SZG_STD_LB_BASE_I=$(($BASE_SZG_STD_LB+$STD_LB_LOOP_CNT))
...
SZG_TRX2_MIO_LB_BASE_O=$BASE_SZG_TRX2_MIO_LB
SZG_TRX2_MIO_LB_BASE_I=$(($BASE_SZG_TRX2_MIO_LB+2))
...
SZG_TRX2_PL_LB_BASE_O=$BASE_SZG_TRX2_PL_LB
SZG_TRX2_PL_LB_BASE_I=$(($BASE_SZG_TRX2_PL_LB+$TRX2_LB_LOOP_CNT))

ALL_MIO_LB_O=($SZG_TRX2_MIO_LB_BASE_O $(($SZG_TRX2_MIO_LB_BASE_O + 1)) $MIO13 $MIO17 $MIO21 $MIO23 $MIO36 $MIO27 $MIO29)
ALL_MIO_LB_I=($SZG_TRX2_MIO_LB_BASE_I $(($SZG_TRX2_MIO_LB_BASE_I + 1)) $MIO14 $MIO18 $MIO22 $MIO35 $MIO37 $MIO28 $MIO30)
...
do
   export_gpio $(($SZG_STD_LB_BASE_O + $i)) "out" 0
   export_gpio $(($SZG_STD_LB_BASE_I + $i)) "in"
done
...
for (( i=0; i<$TRX2_LB_LOOP_CNT; i++ ))
do
   export_gpio $(($SZG_TRX2_PL_LB_BASE_O + $i)) "out" 0
   export_gpio $(($SZG_TRX2_PL_LB_BASE_I + $i)) "in"
done
...
for (( i=0; i<$STD_LB_LOOP_CNT; i++ ))
do
   echo 1 > gpio$((SZG_STD_LB_BASE_O + $i))/value

   if [ $DEBUG -eq 1 ]; then echo -e "\ndebug: writing 1 to output gpio $i (number $((SZG_STD_LB_BASE_O + $i)))"; fi

   for (( j=0; j<$STD_LB_LOOP_CNT; j++ ))
   do
      read_bit=$(cat gpio$((SZG_STD_LB_BASE_I + $j))/value) || read_bit=-1

      if [ $DEBUG -eq 1 ]; then echo "debug: reading $read_bit from input gpio $j (number $((SZG_STD_LB_BASE_I + $j)))"; fi

      if [ $i -eq $j ]
      then
         if [ $read_bit -ne 1 ]
         then
            echo "error: bit $j of SZG_STD_LB_BASE_I should be set"
            error_count=$((error_count+1))
         fi
      else
         if [ $read_bit -ne 0 ]
         then
            echo "error: bit $j of SZG_STD_LB_BASE_I should not be set"
            error_count=$((error_count+1))
         fi
      fi
   done
...
for (( i=0; i<$TRX2_LB_LOOP_CNT; i++ ))
do
   echo 1 > gpio$((SZG_TRX2_PL_LB_BASE_O + $i))/value

   if [ $DEBUG -eq 1 ]; then echo -e "\ndebug: writing 1 to output gpio $i (number $((SZG_TRX2_PL_LB_BASE_O + $i)))"; fi

   for (( j=0; j<$TRX2_LB_LOOP_CNT; j++ ))
   do
      read_bit=$(cat gpio$((SZG_TRX2_PL_LB_BASE_I + $j))/value) || read_bit=-1

      if [ $DEBUG -eq 1 ]; then echo "debug: reading $read_bit from input gpio $j (number $((SZG_TRX2_PL_LB_BASE_I + $j)))"; fi

      if [ $i -eq $j ]
      then
         if [ $read_bit -ne 1 ]
         then
            echo "error: bit $j of SZG_TRX2_PL_LB_BASE_I should be set"
            error_count=$((error_count+1))
         fi
      else
         if [ $read_bit -ne 0 ]
         then
            echo "error: bit $j of SZG_TRX2_PL_LB_BASE_I should not be set"
            error_count=$((error_count+1))
         fi
      fi
   done
...
for (( i=0; i<$STD_LB_LOOP_CNT; i++ ))
do
   unexport_gpio $(($SZG_STD_LB_BASE_O + $i))
   unexport_gpio $(($SZG_STD_LB_BASE_I + $i))
done
...
for (( i=0; i<$TRX2_LB_LOOP_CNT; i++ ))
do
   unexport_gpio $(($SZG_TRX2_PL_LB_BASE_O + $i))
   unexport_gpio $(($SZG_TRX2_PL_LB_BASE_I + $i))
done
